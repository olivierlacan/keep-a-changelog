...
- Link previews (OpenGraph images) for every language and version, each in
  the design of that version's site, generated from the page's own title and
  description by `bin/rake og:images`. Before, every page shared one English
  image with the retired tagline.
